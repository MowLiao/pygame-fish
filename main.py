################################################################
## A game created in pygame for a Computer Science Level 1    ##
## Core Programming coursework, demonstrating use and         ##
## knowledge of classes, functions and pygame modules.        ##
##                                                            ##
## Author  : Melissa Liau                                     ##
## Created : 2015-01-26                                       ##
## Updated : 2015-02-14                                       ##
...
import sprites
import random
import variables  # file containing default variables used within game
...
while running:
    ...
    if game_initialise:
        ...
        player = sprites.player_fish(screen,"pfish","player02.png")
        player_fish = pygame.sprite.Group(player)
        enemy_fishies = pygame.sprite.Group()
        ...
        starttime = time()
        fishmovenum = 1 + variables.enemyvelocity
        fishmovetick = 0
        fishexist = False
        updatetup = ()
        ...
        allrates = variables.enemyfishtypespawn
        game_initialise = False
        game_running = True
    ...
    while game_running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
                print("Quitting...")
                enemy_fishies.clear(screen, background)
                player_fish.clear(screen, background)
                background_loop.clear(screen, background)
                menugroup.clear(screen, background)
                for img in enemy_fishies:
                    img.kill()
                for img in player_fish:
                    img.kill()
        ...
        # Randomly spawn fish of a random type depending on rates in variables.enemyfishtypespawn.
        # Adds to enemy_fishies group.
        spawn = random_generator(1000,-1000 + (10 * variables.enemyfishspawnrate)) 
        if  spawn > 0:
            ...
            variables.enemyfishspawnrate = spawnrates[11]
            tmp = random_generator(10,0,0)
     
            for r in rates:
                if tmp < r:
                    break
            fishnum = rates.index(r)
            
            fish = sprites.enemy_fish(screen, "efish", variables.fishfiles[fishnum],
                                      velocity=(random_generator(1,1,0),random_generator(1,1,0)))
            enemy_fishies.add(fish)
            fishexist = True
        fishmovetick += 1
        ...
        if fishmovetick == fishmovenum and fishexist:
            for efish in enemy_fishies:
                updatetup = efish.update(size,player_fish,player.weight)
                if updatetup != None:
                    player.enemy_collision(updatetup[0],updatetup[1]) # [0] = weight, [1] = nutrition
                    for sizetup in variables.player_fish_size:
                        if sizetup[0] > player.weight:
                            break
                    player.update_image("pfish", "player"+sizetup[1]+".png")
                    eatindicate.update_image("ui", "eatindicate"+sizetup[1]+".png")
                    
        if player.alive:
            # If player does not lose game, continue game.
            if vyvx != [[0,0],[0,0]]:
                player_fish.update(size,vyvx)
            background_loop.draw(screen)        
            enemy_fishies.draw(screen)
            player_fish.draw(screen)

            # Reset fishmovetick to keep fish movement slower.
            if fishmovetick == fishmovenum:
                fishmovetick = 0
